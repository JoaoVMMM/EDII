...
def nodeSearch(node, val):
    if not node: return False
    
    if node.value == val: return node
    
    elif node.value <= val: return nodeSearch(node.right_child, val)
    
    return nodeSearch(node.left_child, val)
...
def validateThreeNodes(bst, valueOne, valueTwo, valueThree):
    ...
    """
    n2 = nodeSearch(bst.root, valueTwo)
    
    descendant = nodeSearch(n2, valueThree) or nodeSearch(n2, valueOne)
    
    if not descendant: return False
    
    elif descendant.value == valueOne: ancestorVal = valueThree
        
    else: ancestorVal = valueOne
        
    ancestor = nodeSearch(bst.root, ancestorVal)
    
    if nodeSearch(ancestor, valueTwo): return True
